...
object MachineEncoder
    extends Machine("Encoder", BlockEncoder)
    with GuiProvider {
  ...
  override def getGui(te: TEClass, player: EntityPlayer) = new GuiEncoder(
    new ContainerEncoder(te, player)
  )
  ...
  NetHandler.regServerHandler {
    case (MsgSetRecipe(recipe), player) =>
      player.addChatMessage(
        new ChatComponentText("Your client version of AE2 Stuff, please update")
          .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED))
      )

    case (MsgSetRecipe2(recipe), player) =>
      Misc
        .asInstanceOpt(player.openContainer, classOf[ContainerEncoder])
        .foreach { cont =>
          for ((slotNum, recIdx) <- cont.te.slots.recipe.zipWithIndex) {
            if (recipe.isDefinedAt(recIdx))
              cont.te.setInventorySlotContents(
                recIdx,
                cont.te.findMatchingRecipeStack(recipe(recIdx) map (_.stack))
              )
            else
    ...
    case (MsgSetRecipe3(data), player) =>
      Misc
        ...
        .foreach { cont =>
          for ((slotNum, recIdx) <- cont.te.slots.recipe.zipWithIndex) {
            val items = data.tag.getList[NBTTagCompound](
              slotNum.toString
            ) map ItemStack.loadItemStackFromNBT
            if (items.nonEmpty)
              cont.te.setInventorySlotContents(
                recIdx,
                cont.te.findMatchingRecipeStack(items)
              )
            else
              cont.te.setInventorySlotContents(recIdx, null)
          }
