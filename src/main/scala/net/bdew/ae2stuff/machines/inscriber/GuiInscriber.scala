...
class GuiInscriber(cont: ContainerInscriber)
    extends BaseScreen(cont, if (cont.hasToolbox) 246 else 211, 176) {
  ...
  override def initGui() {
    ...
    widgets.add(
      new WidgetFillDataSlot(
        new Rect(135, 39, 6, 18),
        AE2Textures.inscriberProgress,
        Direction.UP,
        cont.te.progress,
        1f
      ) {
        override def handleTooltip(p: Point, tip: mutable.MutableList[String]) =
          tip += "%.0f".format(cont.te.progress * 100) + "%"
      }
    )
    widgets.add(
      WidgetSlotLock(Rect(45 - 11, 16 + 4, 8, 8), cont.te.topLocked, "top")
    )
    widgets.add(
      WidgetSlotLock(
        Rect(45 - 11, 62 + 4, 8, 8),
        cont.te.bottomLocked,
        "bottom"
      )
    )
  }
  ...
  override protected def drawGuiContainerBackgroundLayer(
    ...
    widgets.drawTexture(
      upgradesRect + rect.origin,
      AE2Textures.upgradesBackground5
    )
    if (cont.hasToolbox)
      widgets.drawTexture(
        toolBoxRect + rect.origin,
        AE2Textures.toolBoxBackground
      )
  }
}
